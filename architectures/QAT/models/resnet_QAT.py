...
class ResNet_QAT(nn.Module):
    def __init__(self, block, layers, target_dataset, groups=1, width_per_group=64, replace_stride_with_dilation=None, config=None):
        ...
        if config is None:
            raise RuntimeError('configuration needs for initializing QAT model!')
        self.config = config
        ...
        for m in self.modules():
            if isinstance(m, Bottleneck_QAT):
...
def resnet18_QAT(target_dataset, config=None):
    return ResNet_QAT(BasicBlock_QAT, [2, 2, 2, 2], target_dataset, config=config)


def resnet34_QAT(target_dataset, config=None):
    return ResNet_QAT(BasicBlock_QAT, [3, 4, 6, 3], target_dataset, config=config)
